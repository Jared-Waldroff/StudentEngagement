...
class Drawer:
    ...
    def draw_axis(self, image, pose, landmarks, bbox, emotion_category, face_id, axis_size):
        """
        ...
        for i, lms in enumerate(landmarks):
            if lms is not None:
                ...
                if (pitch > 30 or pitch < -50) or (yaw > 55 or yaw < -55) or emotion_category == "Negative":
                    engaged_text = 'Not Engaged'

                # Draw 'ID' and 'Engaged' text further above the bounding box with a blue background
                if "face" in bbox:
                    x1, y1, x2, y2 = bbox["face"]

                    # Define offsets to position the box further above the probability box
                    box_y_offset = 54  # Adjust position of box
                    text_y_offset = 50  # Adjust the text position inside the box

                    # Background for ID and Engagement status
                    text = f"ID:{face_id} {engaged_text}"
                    font = cv2.FONT_HERSHEY_SIMPLEX
                    font_scale = 0.75
                    font_thickness = 2
                    text_color = (255, 255, 255)  # White text
                    bg_color = (255, 150, 0)  # Blue background

                    # Get text size
                    (text_width, text_height), _ = cv2.getTextSize(text, font, font_scale, font_thickness)

                    # Draw the blue rectangle
                    cv2.rectangle(image, (int(x1), int(y1) - box_y_offset),
                                  (int(x1) + text_width + 10, int(y1) - box_y_offset + text_height + 10), bg_color, -1)

                    # Draw the text on top of the blue rectangle
                    cv2.putText(image, text, (int(x1) + 5, int(y1) - text_y_offset + text_height),
                                font, font_scale, text_color, font_thickness, cv2.LINE_AA)

                    # Draw Emotion label below the bounding box
                    cv2.putText(image, f"Emotion: {emotion_category}", (int(x1), int(y2) + 25),
                                font, 0.75, (255, 255, 255), 2)

                    # Display yaw, pitch, and roll angles
                    cv2.putText(image, f"yaw: {yaw:.2f}", (int(x1), int(y2) + 50), font, 0.75,
                                (0, 255, 0), 2)
                    cv2.putText(image, f"pitch: {pitch:.2f}", (int(x1), int(y2) + 75), font,
                                0.75, (255, 0, 0), 2)
                    cv2.putText(image, f"roll: {roll:.2f}", (int(x1), int(y2) + 100), font, 0.75,
                                (0, 0, 255), 2)
